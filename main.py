    ...
    with open(csv_file, 'w', newline='') as file:
        ...
        for channel in guild.text_channels:
            ...
            async for message in channel.history(limit=None):
                if message.created_at < datetime.datetime.now() - datetime.timedelta(days=1):
                    break
                writer.writerow({
...
def summarize_discord_chat(csv_file):
    ...
    print("Summarized discord chat.")

    return channel_summary_dict

# Discordチャンネルにメッセージを送信する関数を定義
async def send_message_to_discord(bot, channel_id, message, file_path):
    # チャンネルIDからチャンネルオブジェクトを取得
    target_channel = bot.get_channel(int(channel_id))

    # メッセージを送信
    await target_channel.send(message, file=discord.File(file_path))
...
if __name__ == "__main__":
    ...
    async def on_ready():
        ...
        for channel, data in summaries.items():
            ...
            await send_message_to_discord(bot, summary_channel_id, message, csv_file)
